...
def test_zero_qualified_cases_produces_no_qualified_message():
    result = PdfRunResult(detected_cases=2, qualified_cases=0)
    notes = build_result_notifications(result)
    assert notes == [NO_QUALIFIED_MSG]
...
def test_channel_send_failed_message():
    result = PdfRunResult(qualified_cases=1, channel_send_failed=True)
    assert build_result_notifications(result) == [CHANNEL_FAIL_MSG]


def test_success_message_includes_counts():
    result = PdfRunResult(qualified_cases=1, cases_sent=1, images_sent=5, notion_upserted=1)
    notes = build_result_notifications(result)
    assert notes == ["처리 완료: 1건 전송, 5개 이미지 생성, 노션 1건 업데이트"]


def test_notion_failure_prepended_before_success_message():
    result = PdfRunResult(qualified_cases=1, cases_sent=1, images_sent=3, any_notion_failed=True)
    notes = build_result_notifications(result)
...
def test_partial_image_failure_appends_extra_note():
    result = PdfRunResult(qualified_cases=1, cases_sent=1, images_sent=4, images_failed=2)
    notes = build_result_notifications(result)
